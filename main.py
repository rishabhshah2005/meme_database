...
def insert_stuff(id, temp, typ, ups, comms, awds):
    cur.execute(f"insert into memes_posted values({id}, '{temp}', '{typ}', {ups}, {comms}, {awds})")
    con.commit()
...
def change(id, temp, typ, ups, comms, awds):
    cur.execute(f"update memes_posted set template='{temp}', type='{typ}', upvotes={ups}, comments={comms}, awards={awds} where id={id}")
    con.commit()
...
def delete(win):
    def display(id, frm):
        ...
        try:
            ...
            Label(frm, text='Done', font=('none', 14, 'bold'), padx=5, pady=5).pack()

        except:
            Label(frm, text='ERROR', font=('none', 14, 'bold'), padx=5, pady=5).pack()
...
def update(win):
    def display(frm, id, temp, typ, ups, comms, awds):
        ...
        Label(frm, text=f"ID-{id} UPDATED", font=('none', 19, 'bold'), padx=7, pady=7).pack()

    clear_frame(win)
    frm = Frame(win)
    frm.grid(row=7, column=0, columnspan=3)
    Label(win, text="ID", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=0, column=0, pady=10)
    Label(win, text="Template", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=1, column=0, pady=10)
    Label(win, text="Type", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=2, column=0, pady=10)
    Label(win, text="Upvotes", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=3, column=0, pady=10)
    Label(win, text="Comments", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=4, column=0, pady=10)
    Label(win, text="Awards", padx=8, pady=5, font=('none', 15, 'bold')).grid(row=5, column=0, pady=10)

    id_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    id_.grid(column=1, row=0, pady=10)
    temp_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    temp_.grid(column=1, row=1, pady=10)
    typ_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    typ_.grid(column=1, row=2, pady=10)
    ups_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    ups_.grid(column=1, row=3, pady=10)
    comms_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    comms_.grid(column=1, row=4, pady=10)
    awds_ = Entry(win, width=17, justify=RIGHT, borderwidth=5, font=('none', 20))
    awds_.grid(column=1, row=5, pady=10)


    Button(win, text="Update", font=('none', 20, 'bold'), padx=7, pady=10, command=lambda: display(frm, id_.get(), temp_.get(), typ_.get(), ups_.get(), comms_.get(), awds_.get())).grid(row=0, column=2, rowspan=3, padx=40)
    Button(win, text="HOME", font=('none', 20, 'bold'), padx=7, pady=10, command=lambda: home(root)).grid(row=3, column=2, rowspan=3, padx=40)
...
def insert(win):
    def display(frm, id, temp, typ, ups, comms, awds):
        ...
        try:
            ...
            Label(frm, text=f"ID-{id} INSERTED", font=('none', 19, 'bold'), padx=7, pady=7).pack()
        except:
            Label(frm, text="ERROR", font=('none', 19, 'bold'), padx=7, pady=7).pack()
    ...
    Button(win, text="Insert", font=('none', 20, 'bold'), padx=7, pady=10, command=lambda: display(frm, id_.get(), temp_.get(), typ_.get(), ups_.get(), comms_.get(), awds_.get())).grid(row=0, column=2, rowspan=3, padx=40)
    Button(win, text="HOME", font=('none', 20, 'bold'), padx=7, pady=10, command=lambda: home(root)).grid(row=3, column=2, rowspan=3, padx=40)
...
def home(root):
    ...
    delete_ = Button(root, text='Delete by ID', padx=20, pady='20', font=(
        'none', 15, 'bold'), command=lambda: delete(root)).grid(row=1, column=0, pady=20)
    update_meme = Button(root, text='Update Data', padx=20, pady='20', font=(
        'none', 15, 'bold'), command=lambda: update(root)).grid(row=1, column=1, pady=20)
    ...
    insert_ = Button(root, text='Insert Data', padx=20, pady='20', font=(
        'none', 15, 'bold'), command=lambda: insert(root)).grid(row=2, column=1, pady=20)
